...
import { isFirebaseConfigured, uploadToFirebaseStorage } from '../services/firebaseService.js';
...
router.post('/', strictUploadLimiter, (req, res, next) => {
  upload.single('file')(req, res, (err) => {
    if (err) {
      console.error("Multer error:", err);
      return res.status(400).json({ success: false, error: err.message === 'File too large' ? 'File exceeds maximum allowed size (50MB)' : err.message });
    }
    next();
  });
}, async (req, res) => {
  try {
    ...
    // 1. Try Firebase Storage if configured (Low Cost Cloud Storage for Media)
    if (isFirebaseConfigured()) {
      try {
        const firebaseRes = await uploadToFirebaseStorage({ filename: originalname, mimetype, buffer });
        return res.json({ success: true, url: firebaseRes.url, storage: 'firebase' });
      } catch (fbErr) {
        console.warn("Firebase upload fallback to DB/Memory:", fbErr.message);
      }
    }

    // 2. Try PostgreSQL storage
    if (isDbConnected) {
      try {
        ...
        return res.json({ success: true, fileId: result.rows[0].id, url: `/api/files/${result.rows[0].id}`, storage: 'postgres' });
      } catch (dbErr) {
    ...
    return res.json({ success: true, fileId, url: `/api/files/${fileId}`, storage: 'memory' });
